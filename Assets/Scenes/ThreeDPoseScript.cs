...
public class ThreeDPoseScript : MonoBehaviour
{
    ...
    private readonly int CameraFps = 30;
    private WebCamTexture webCamTexture;
    private Quaternion camTextureBaseRotation;
    ...
    void Start()
    {
        ...
        private IEnumerator GetFilePath()
        {
            ...
            var getFilePathAsync_0_Coroutine = Utils.getFilePathAsync ("MobileNet3D2.onnx", (result) => {
...
#if UNITY_EDITOR
        webCamTexture = new WebCamTexture(devices[0].name, CameraWidth, CameraHeight, CameraFps);
#elif UNITY_IPHONE
        webCamTexture = new WebCamTexture(devices[1].name, CameraWidth, CameraHeight, CameraFps);
#else
        webCamTexture = new WebCamTexture(devices[0].name, CameraWidth, CameraHeight, CameraFps);
#endif

        GameObject videoScreen = GameObject.Find("VideoScreen");
        screen = videoScreen.GetComponent<RawImage>();
        var sd = screen.GetComponent<RectTransform>();
        camTextureBaseRotation = screen.transform.rotation;
        screen.texture = webCamTexture;

        webCamTexture.Play();

        sd.sizeDelta = new Vector2(videoScreenWidth, (int)(videoScreenWidth * CameraHeight / CameraWidth));

        texture = new Texture2D(CameraWidth, CameraHeight, TextureFormat.RGBA32, false);
    }
    ...
    private void VideoPlayStart()
    {
        ...
        //videoPlayer.url = Application.streamingAssetsPath + "wiper.mp4";
    ...
    {
        //Debug.LogError("PoseUpdate method");
    ...
    {
        Onnx = Dnn.readNetFromONNX(model_filepath);
    ...
    {
        var blob = Dnn.blobFromImage(img, 1.0 / 255.0, new Size(inputImageSize, inputImageSize), new Scalar(0.5, 0.5, 0.5), false, false);
        Onnx.setInput(blob);
